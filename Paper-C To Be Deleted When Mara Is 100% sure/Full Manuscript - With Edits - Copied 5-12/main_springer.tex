...
generalized Gatheral double mean-reverting (gDMR) dynamics. Since the variance
...
\section{Model, Bermudan problem, and well-posedness}\label{sec:model}

This section introduces the generalized gDMR model and the Bermudan recursion.
The equations for the variance factors do not involve the asset price, while
...
\subsection{Generalized gDMR dynamics}
...
risk-neutral measure $\Q$ with deterministic money-market account $B_t=e^{rt}$. The
...
the covariance matrix of the two Brownian drivers of the variance factors is
invertible in the projection used in Section~\ref{sec:conditional_pde}.

Let $S_0>0$, $v_0\ge0$, and $v'_0\ge0$. We consider the generalized gDMR dynamics
...
\subsection{Bermudan stopping problem}

Let
$$
\pi=\{0=t_0<t_1<\cdots<t_M=T\}
$$
be the Bermudan exercise grid. At $t_n$, the payoff is $h_n(S_{t_n})$,
where $h_n:(0,\infty)\to\R$ is Borel. We assume
$$
\E^\Q\!\left[
...
\section{Conditional hybrid representation for the generalized gDMR model}\label{sec:conditional_pde}

This section derives the conditional representation used by the hybrid method.
The general convergence theorem is taken from Farahany, Jackson, and
Jaimungal~\cite{farahany2020mixing}; our role here is to identify the
model-specific quantities needed to apply it. The variance equations do not
involve $S$, so over one Bermudan interval we condition on the Brownian
increments driving the variance factors. After projecting the asset Brownian
driver onto these increments, at most one residual Brownian component remains.
...
Thus, for an asset-space next-step value function $F$ and
$G(y,v,w):=F(e^y,v,w)$, the pathwise pre-surface is
$$
...
in \cite{farahany2020mixing}. We use it here after checking the model-specific
conditions.
...
regression step, and time-zero estimator. These numerical choices are separate
from the convergence statement quoted in Section~\ref{sec:conditional_pde}.
...
initial asset price, and the direct estimator is
$$
\widehat V_{d,0}(S_0)
=
\max\left\{
...
We report direct Bermudan put pricing experiments for the fixed parameter set in
Table~\ref{tab:parameter-setup}. The plain LSMC estimator
\cite{longstaff2001valuing} is compared with the Hybrid LSMC--PDE estimator
described in Section~\ref{sec:hybrid-method}, using direct prices and
