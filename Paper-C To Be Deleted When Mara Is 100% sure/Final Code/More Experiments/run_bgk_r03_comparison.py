...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run BGK r=0.03 benchmark vs hybrid comparisons.")
    parser.add_argument(
        "--smoke",
        action="store_true",
...
def build_summary(
    ...
    return "\n".join(
        [
            f"# BGK gDMR Comparison with `r=0.03` ({run_label})",
            "",
...
def main() -> None:
    ...
    csv_name = "bgk_r03_smoke_table.csv" if args.smoke else "bgk_r03_comparison_table.csv"
    summary_name = "bgk_r03_smoke_summary.md" if args.smoke else "bgk_r03_comparison_summary.md"
