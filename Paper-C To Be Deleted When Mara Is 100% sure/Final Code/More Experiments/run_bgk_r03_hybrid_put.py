...
This local experiment copy uses the BGK model block by default, but sets
the default rate to r=0.03 and reads the full model block from GDMR_*
...
N = env_int("GDMR_HYBRID_PATHS", 20_000)
N_low = env_int("GDMR_HYBRID_LOW_PATHS", 20_000)
...
def hybrid_prices() -> dict[str, float | int | bool]:
    ...
    return {
        "method": "Hybrid LSMC-PDE with FFT",
