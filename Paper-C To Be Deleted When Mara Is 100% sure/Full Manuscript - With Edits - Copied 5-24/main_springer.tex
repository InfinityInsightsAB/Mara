...
literature. The Hybrid LSMC--PDE estimator is compared with plain
...
The CEV generalization creates a boundary issue at zero, because the variance
diffusion coefficients are not Lipschitz at the boundary when the exponents are
below one. Mishura, Pilipenko, and Ralchenko~\cite{mishura2025gatheral}
studied well-posedness and boundary behavior for a closely related double-CEV
Gatheral volatility system. They prove strong existence, uniqueness,
nonnegativity, and the strong Markov property, and they also study near-zero
behavior and a Skorokhod-reflected version of the model. We cite their work
because it addresses the same boundary issue for Gatheral-type double
mean-reverting variance dynamics. In the present paper we keep the original
non-reflected GDMR dynamics. The proof is included to verify the precise
property used in the pricing construction, namely that the model in
\eqref{eq:gdmr} is strongly well posed and preserves the nonnegative variance
state space.
...
Write $x^+=\max\{x,0\}$. For $\delta\in[1/2,1]$, the inequality
$$
...
The state space for the original model is
$$
...
The proof is given in Appendix~\ref{app:proof-wellposed}. The positive-part
extension of the variance diffusion coefficients is introduced there, at the
point where it is used. The proof follows the triangular structure of the model.
...
Subsection~\ref{sec:hybrid-overview} and
Proposition~\ref{prop:conditional-gaussian-step}, where the Hybrid LSMC--PDE
step conditions on the Brownian increments driving the variance factors.
...
backward recursion. At maturity this surface is initialized by
$$
...
estimator with our Hybrid LSMC--PDE estimator, using large-simulation plain
LSMC estimates as reference prices. The plain LSMC estimator
...
For an estimator $V$ and reference value $V^{\mathrm{ref}}$, we report
$$
...
Euler steps and $1{,}200{,}000$ simulation paths. They are used as numerical
reference values, not as exact prices. The reported standard errors are
empirical Monte Carlo standard errors of the estimators, and the
95\% confidence intervals are computed as the estimate plus or minus
...
the estimator confidence intervals through
$$
...
reference errors. The corresponding price grids are reported in
Appendix~\ref{app:price-grids}.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Figures/step_sweep_20k_direct_relative_error.pdf}
\caption{Reference-relative errors for the matched $20{,}000$-path step-sweep experiment. The reference values are the LSMC estimates in Table~\ref{tab:reference-prices}. Error bars are obtained from the 95\% confidence intervals of the estimators.}
...
errors, and the price grids are reported in
Appendix~\ref{app:price-grids}. The Hybrid LSMC--PDE estimator does not give a
smaller reference error in every step-count configuration, but it gives smaller
reference errors in most of the reported configurations, with the clearest
...
\caption{Reference-relative errors for the matched $60{,}000$-path step-sweep experiment. The reference values are the LSMC estimates in Table~\ref{tab:reference-prices}. Error bars are obtained from the 95\% confidence intervals of the estimators.}
...
corresponding pricing estimates are reported in Appendix~\ref{app:price-grids}.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Figures/path_sweep_steps48_direct_relative_error.pdf}
\caption{Reference-relative errors for the fixed $48$-step path-sweep experiment. The reference values are the LSMC estimates in Table~\ref{tab:reference-prices}. Error bars are obtained from the 95\% confidence intervals of the estimators.}
...
path-count experiment at $60$ Euler steps, with the pricing estimates reported in
Appendix~\ref{app:price-grids}. The reduction in reference error is most
...
\caption{Reference-relative errors for the fixed $60$-step path-sweep experiment. The reference values are the LSMC estimates in Table~\ref{tab:reference-prices}. Error bars are obtained from the 95\% confidence intervals of the estimators.}
...
made against the LSMC reference values in Table~\ref{tab:reference-prices}, not exact prices. The full price
grids are reported in Appendix~\ref{app:price-grids}.
...
The numerical experiments compare the Hybrid LSMC--PDE estimator with plain LSMC
under one fixed GDMR parameter set, based on a calibrated double mean-reverting
specification from the literature. The Hybrid LSMC--PDE
estimator gives smaller reference errors in most of the reported fixed-step and
fixed-path configurations, with the clearest gains at low and moderate path
budgets. The reference values are
...
\noindent\textit{Proof.}
We first work with auxiliary equations on $\R$. For this purpose, the variance
diffusion coefficients are extended by
$$
\sigma_i(x)=\xi_i(x^+)^{\delta_i},\qquad
x^+=\max\{x,0\},\qquad i=1,2.
$$
Once nonnegativity is proved, this extension is inactive because both variance
factors stay in $[0,\infty)$. The auxiliary equations then coincide with the
original system on the state space $\mathcal D$.
...
This appendix reports the Bermudan put pricing estimates corresponding to the numerical experiments in Section~\ref{sec:numerical}. All entries are rounded to three decimal places. The label LSMC denotes the plain least-squares Monte Carlo estimator, and Hybrid denotes the Hybrid LSMC--PDE estimator.

\subsection{Reference prices}

\begin{table}[htbp]
\centering
\caption{Reference prices for the parameter set in Table~\ref{tab:parameter-setup}. Prices are rounded to three decimal places.}
