...
OUTPUT_STEM = "bgk_r00_t1_nex12_path_sweep_atm_steps48_direct_ref1200_paths1200000"
OUTPUT_TABLE = THIS_DIR / f"{OUTPUT_STEM}_table.csv"
OUTPUT_SUMMARY = THIS_DIR / f"{OUTPUT_STEM}_summary.md"
OUTPUT_FIGURE = THIS_DIR / f"{OUTPUT_STEM}_relative_error_with_ci.svg"
CI_Z = 1.96
...
def load_reference() -> dict[str, float]:
    ...
    raise RuntimeError(f"Could not find ATM reference row in {REFERENCE_TABLE.name}")


def load_rows(reference_direct_price: float) -> list[dict[str, float | str]]:
...
def render_figure(rows: list[dict[str, float | str]], reference: dict[str, float]) -> None:
    width = 1260
    height = 760
    ...
    lines = [
        ...
        f'<text x="72" y="88" font-family="{font}" font-size="31" font-weight="700" fill="#17202a">ATM: Direct relative error vs path count</text>',
        ...
        f'<text x="72" y="146" font-family="{font}" font-size="14" fill="#64748b">This reuses the saved ATM 48-step path sweep and recomputes direct relative errors against the updated benchmark reference.</text>',
...
def build_summary(rows: list[dict[str, float | str]], reference: dict[str, float]) -> str:
    table_rows = []
    for row in rows:
        table_rows.append(
            [
    ...
    return "\n".join(
        [
            ...
            "",
            "- Scenario: `ATM`",
            "- Fixed benchmark direct reference: "
            f"`{reference['benchmark_direct_price']:.6f}`",
            "- Fixed benchmark direct reference SE: "
            ...
            "",
            f"![ATM direct relative error rebased to 1200-step benchmark]({OUTPUT_FIGURE})",
            "",
...
def main() -> None:
    ...
    rows = load_rows(reference["benchmark_direct_price"])
    write_csv(rows)
    render_figure(rows, reference)
    OUTPUT_SUMMARY.write_text(build_summary(rows, reference), encoding="utf-8")
