...
REFERENCE_TABLE_MAIN = THIS_DIR / "bgk_r00_t1_nex12_benchmark_steps1200_paths1200000_table.csv"
REFERENCE_TABLE_DEEP_OTM = THIS_DIR / "bgk_r00_t1_nex12_benchmark_steps1200_paths1200000_k80_k70_table.csv"

CI_Z = 1.96
STEP_COUNTS = [24, 48, 72, 96]
DEFAULT_MATCHED_PATHS = 60000

SCENARIOS = {
    "itm110": {"slug": "itm110", "label": "ITM put", "GDMR_STRIKE": "110.0", "GDMR_MATURITY": "1.0"},
    "atm100": {"slug": "atm100", "label": "ATM", "GDMR_STRIKE": "100.0", "GDMR_MATURITY": "1.0"},
    "otm90": {"slug": "otm90", "label": "OTM put", "GDMR_STRIKE": "90.0", "GDMR_MATURITY": "1.0"},
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one BGK 12-date same-path step sweep for a single strike and report direct error, SE, CI width, and runtime."
    )
    parser.add_argument("--scenario", choices=sorted(SCENARIOS), required=True)
    parser.add_argument("--paths", type=int, default=DEFAULT_MATCHED_PATHS)
...
def output_stem(scenario_slug: str, path_count: int) -> str:
    return f"bgk_r00_t1_nex12_step_sweep_{scenario_slug}_ref1200_direct_samepaths{paths_tag(path_count)}_s24487296"


def summary_path(scenario_slug: str, path_count: int) -> Path:
    return THIS_DIR / f"{output_stem(scenario_slug, path_count)}_summary.md"


def table_path(scenario_slug: str, path_count: int) -> Path:
    return THIS_DIR / f"{output_stem(scenario_slug, path_count)}_table.csv"


def scratch_dir(scenario_slug: str, path_count: int) -> Path:
    return THIS_DIR / f"_scratch_{output_stem(scenario_slug, path_count)}"


def ci_width(se: float) -> float:
    return 2.0 * CI_Z * se
...
def load_reference_map() -> dict[str, dict[str, float | None]]:
    data: dict[str, dict[str, float | None]] = {}

    with REFERENCE_TABLE_MAIN.open("r", newline="", encoding="utf-8") as handle:
        reader = csv.DictReader(handle)
        for row in reader:
            direct = float(row["benchmark_direct_price"])
            se = float(row["benchmark_direct_error"])
            half_width = CI_Z * se
            data[row["scenario"]] = {
                "benchmark_direct_price": direct,
                "benchmark_direct_error": se,
                "benchmark_direct_ci_lower": direct - half_width,
                "benchmark_direct_ci_upper": direct + half_width,
                "runtime_seconds": None,
            }

    with REFERENCE_TABLE_DEEP_OTM.open("r", newline="", encoding="utf-8") as handle:
        reader = csv.DictReader(handle)
        for row in reader:
            data[row["scenario"]] = {
...
def build_env(
    ...
    if method == "benchmark":
        ...
        env.update(
            {
                "GDMR_LSMC_PATHS": str(path_count),
                "GDMR_LSMC_LOW_PATHS": str(path_count),
                "GDMR_LSMC_SEED": "2026",
        ...
        env.update(
            {
                "GDMR_HYBRID_PATHS": str(path_count),
                "GDMR_HYBRID_LOW_PATHS": str(path_count),
...
def method_row(
    ...
    return {
        ...
        "reference_direct_price": reference_price,
        "price_direct": direct_price,
        "se_direct": direct_se,
        "ci_width_direct": ci_width(direct_se),
        "rel_error_direct": rel_error(direct_price, reference_price),
    }
...
def build_summary(
    ...
    reference_runtime_text = f"`{reference_runtime:.2f} s`" if reference_runtime is not None else "`not recorded in saved reference file`"
    ...
    for row in rows:
        table_rows.append(
            [
                ...
                f"`{float(row['ci_width_direct']):.6f}`",
    ...
    lines = [
        f"# BGK 12-date {scenario['label']} step sweep with matched {path_count:,} paths and steps 24, 48, 72, 96",
        "",
        ...
        markdown_table(
            ...
            [[
                f"`{scenario['label']}`",
                f"`{float(reference['benchmark_direct_price']):.6f}`",
                f"`{float(reference['benchmark_direct_error']):.6f}`",
                f"`[{float(reference['benchmark_direct_ci_lower']):.6f}, {float(reference['benchmark_direct_ci_upper']):.6f}]`",
                reference_runtime_text,
            ]],
        ),
        "",
        "## Sweep settings",
        ...
        f"- LSMC paths: `{path_count}`",
        f"- Hybrid paths: `{path_count}`",
    ...
    lines.append(
        f"- Total runtime across these four steps is `{sum(float(benchmark_rows[step]['runtime_seconds']) for step in STEP_COUNTS):.2f} s` for LSMC versus `{sum(float(hybrid_rows[step]['runtime_seconds']) for step in STEP_COUNTS):.2f} s` for Hybrid."
    )
    lines.append("")
    lines.append(f"Saved CSV: `{table_path(scenario['slug'], path_count).name}`")
    return "\n".join(lines)
...
def main() -> None:
    ...
    rows: list[dict[str, float | str]] = []
    run_scratch_dir = scratch_dir(scenario["slug"], args.paths)
    success = False
    try:
        for step_count in STEP_COUNTS:
            benchmark, benchmark_runtime = run_benchmark(
                build_env(step_count, scenario, "benchmark", run_scratch_dir, args.paths)
            )
            benchmark_row = method_row(
                scenario,
                step_count,
                "benchmark",
                benchmark_runtime,
                ...
                float(reference["benchmark_direct_price"]),
            )
            rows.append(benchmark_row)

            hybrid, hybrid_runtime = run_hybrid(
                build_env(step_count, scenario, "hybrid", run_scratch_dir, args.paths)
            )
            hybrid_row = method_row(
                scenario,
                step_count,
                "hybrid",
                hybrid_runtime,
            ...
            print(
                ...
                f"LSMC price {benchmark_row['price_direct']:.6f}, err {100.0 * float(benchmark_row['rel_error_direct']):.3f}%, "
                f"SE {benchmark_row['se_direct']:.6f}, CI width {benchmark_row['ci_width_direct']:.6f}, runtime {benchmark_row['runtime_seconds']:.2f}s; "
                f"Hybrid price {hybrid_row['price_direct']:.6f}, err {100.0 * float(hybrid_row['rel_error_direct']):.3f}%, "
                f"SE {hybrid_row['se_direct']:.6f}, CI width {hybrid_row['ci_width_direct']:.6f}, runtime {hybrid_row['runtime_seconds']:.2f}s",
                flush=True,
            )

        rows.sort(key=lambda item: (int(item["euler_steps"]), str(item["method"])))
        write_csv(table_path(scenario["slug"], args.paths), rows)
        summary_path(scenario["slug"], args.paths).write_text(
            build_summary(scenario, rows, reference, args.paths),
            encoding="utf-8",
        )
        success = True
    finally:
    ...
    print(f"Summary: {summary_path(scenario['slug'], args.paths).name}")
    print(f"Table:   {table_path(scenario['slug'], args.paths).name}")
