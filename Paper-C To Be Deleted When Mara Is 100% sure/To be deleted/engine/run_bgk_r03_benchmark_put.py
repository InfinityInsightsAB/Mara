...
itm_only_policy = os.environ.get("GDMR_LSMC_ITM_ONLY", "0") == "1"
...
def lsmc_direct_and_coefficients() -> Tuple[float, float, list]:
    ...
    try:
        ...
        for step in range(N_ex - 1, 0, -1):
            ...
            if itm_only_policy:
                itm_mask = exercise_value > 0.0
                if np.count_nonzero(itm_mask) >= x_design.shape[1]:
                    coeff = ridge_regression(x_design[itm_mask], discounted_future[itm_mask], ridge_lambda)
                else:
            ...
            if itm_only_policy:
                exercise_now = itm_mask & exercise_now
...
def lsmc_low_estimator(coeff_steps: list) -> Tuple[float, float]:
    ...
    try:
        ...
        for step in range(1, N_ex):
            ...
            if itm_only_policy:
                exercise_now = exercise_now & (exercise_value > 0.0)
...
def benchmark_prices() -> dict[str, float | int | str]:
    ...
    return {
        ...
        "seed": seed,
        "low_seed": low_seed,
        "lsmc_policy": "itm_only" if itm_only_policy else "all_paths",
