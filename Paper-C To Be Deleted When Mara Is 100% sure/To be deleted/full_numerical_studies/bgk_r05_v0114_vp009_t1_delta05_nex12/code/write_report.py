...
REPORT_MD = SUMMARY_DIR / f"{CASE_ID}_sandbox_rerun_report_20260504.md"
...
def benchmark_rows() -> list[list[str]]:
    ...
    for row in sorted(sandbox, key=lambda item: finite(item["K"])):
        ...
        rows.append([str(k), fmt(price), fmt(se)])
    return rows
...
def require_full_rerun_inputs(config: dict[str, Any], validation: dict[str, Any]) -> None:
    ...
    required_files = [
        ...
        FIGURE_DIR / f"{CASE_ID}_step_sweep_20k_direct_relative_error.pdf",
        FIGURE_DIR / f"{CASE_ID}_step_sweep_60k_direct_relative_error.pdf",
...
def main() -> None:
    ...
    lines: list[str] = []
    lines.append("# Sandbox Rerun Report: BGK Positive-Rate Square-Root Numerical Study")
    lines.append("")
    lines.append(f"This report summarizes the sandbox-only from-scratch rerun for the numerical-study case `{CASE_ID}`. It is a verification artifact for the standalone numerical-study PDF.")
    lines.append("")
    ...
    lines.append("- Assets: step-sweep and path-sweep figures, representative tables, benchmark/setup tables, and appendix-style price tables.")
    lines.append("")
    lines.append("## Benchmark Prices")
    lines.extend(md_table(["K", "Sandbox price", "Sandbox SE"], benchmark_rows()))
    lines.append("")
    ...
    lines.append("")
    lines.append(f"- `figures/{CASE_ID}_step_sweep_20k_direct_relative_error.pdf`")
    lines.append(f"- `figures/{CASE_ID}_step_sweep_60k_direct_relative_error.pdf`")
    lines.append(f"- `figures/{CASE_ID}_path_sweep_steps48_direct_relative_error.pdf`")
    lines.append(f"- `figures/{CASE_ID}_path_sweep_steps60_direct_relative_error.pdf`")
    lines.append(f"- `tables/{CASE_ID}_experimental_setting_table.tex`")
    lines.append(f"- `tables/{CASE_ID}_benchmark_reference_table.tex`")
    lines.append(f"- `tables/{CASE_ID}_step_sweep_20k_step72_table.tex`")
    lines.append(f"- `tables/{CASE_ID}_step_sweep_60k_step72_table.tex`")
    lines.append(f"- `tables/{CASE_ID}_path_sweep_steps48_path20k_table.tex`")
    lines.append(f"- `tables/{CASE_ID}_path_sweep_steps60_path20k_table.tex`")
    lines.append(f"- `tables/{CASE_ID}_appendix_price_tables.tex`")
    lines.append("")
    lines.append("## Notes")
    lines.append("")
    lines.append("- All generated files are intended to stay under the sandbox root.")
    lines.append("- The new pricing engines are separate from the old `run_bgk_*` scripts; all reported values in this study come from the sandbox rerun.")
