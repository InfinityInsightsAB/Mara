...
def write_high_accuracy_table(case_id: str) -> None:
    rows = read_rows(HIGH_PATH_DIR / f"{case_id}_hybrid_highaccuracy_steps1200_paths100000_grid601_table.csv")
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{High-accuracy Hybrid LSMC--PDE prices with \(1200\) steps, \(100{,}000\) paths, and a widened \(601\)-point asset grid.}",
        rf"\label{{tab:{case_id}-hybrid-highaccuracy}}",
        r"\begin{tabular}{lrrrr}",
        r"\toprule",
        r"Strike & Benchmark & Hybrid price & SE & Rel. error \\",
        r"\midrule",
    ]
    for row in sorted(rows, key=lambda item: finite(item["K"])):
        lines.append(
            rf"${int(finite(row['K']))}$ & {finite(row['reference_direct_price']):.6f} & "
            rf"{finite(row['price_direct']):.6f} & {finite(row['se_direct']):.6f} & "
            rf"{100.0 * finite(row['rel_error_direct']):.3f}\% \\"
        )
    lines.extend([r"\bottomrule", r"\end{tabular}", r"\end{table}", ""])
    (TABLE_DIR / f"{case_id}_hybrid_highaccuracy_steps1200_paths100000_grid601_table.tex").write_text(
        "\n".join(lines),
        encoding="utf-8",
    )
...
def write_tex() -> Path:
    ...
    write_high_accuracy_table(case_id)
    ...
    high_accuracy = read_rows(HIGH_PATH_DIR / f"{case_id}_hybrid_highaccuracy_steps1200_paths100000_grid601_table.csv")
    ...
    high_max = max(100.0 * finite(row["rel_error_direct"]) for row in high_accuracy)
    title = "Standalone numerical study for a positive-rate square-root gDMR parameter set"
    tex_path = TEX_DIR / f"main_numerical_study_{case_id}_highpath.tex"
    content = rf"""
...
\subsection{{High-path Hybrid confirmation}}

The fixed-step sweeps show that the coarse \(48\)- and \(60\)-step designs do not by themselves remove the discretization and grid effects for this square-root case. As a final confirmation, the Hybrid LSMC--PDE calculation is repeated with \(1200\) Euler steps, \(100,000\) direct paths, and a widened \(601\)-point asset grid on \([0.10S_0,5S_0]\). Under this accuracy-oriented setting, the largest benchmark-relative Hybrid error over the five strikes is {high_max:.3f}\%, and each strike is below the one-percent level.

{table_input(case_id, "hybrid_highaccuracy_steps1200_paths100000_grid601_table")}

The high-path confirmation indicates that the larger errors observed in the moderate-budget sweeps are not caused by the Bermudan stopping formulation itself. They are primarily a numerical-budget effect for this parameter set, and the Hybrid prices recover the benchmark scale once the time grid and asset grid are refined together with the path budget.
