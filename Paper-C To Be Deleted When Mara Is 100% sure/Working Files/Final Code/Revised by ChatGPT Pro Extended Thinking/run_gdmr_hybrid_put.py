...
if abs(rho23) >= 1.0:
    raise ValueError("rho23 must satisfy |rho23| < 1.")
...
def projection_coefficients() -> tuple[float, float, float]:
    r"""
    Compute \beta_2, \beta_3, \sigma_\perp^2 in

        dW^{(1)} = \beta_2 dW^{(2)} + \beta_3 dW^{(3)} + \sigma_\perp dB^\perp.
    """
...
def vol_basis(
    ...
    Return
        ...
        1_{[0,v_cap]\times[0,v'_cap]}(v,v')
        (1, y, z, y^2, yz, z^2, y^3, y^2 z, y z^2, z^3),

    with y = v / v_cap and z = v' / v'_cap.
    """
...
def ridge_regression_all(x_design: np.ndarray, y_targets: np.ndarray, ridge_value: float) -> np.ndarray:
    r"""
    Solve, for all asset-grid columns at once,

        \arg\min_A ||Y - X A^\top||_F^2 + \lambda ||A||_F^2.
    """
...
def simulate_volatility_statistics(
    ...
    r"""
    On each interval [t_n, t_{n+1}], compute

        A_n = \int (r - v_t/2) dt,
        B_n = \sigma_\perp^2 \int v_t dt,
        Z_n = \int \sqrt{v_t} (\beta_2 dW_t^{(2)} + \beta_3 dW_t^{(3)}),

    so that X_{t_{n+1}} = x + A_n + Z_n + \sqrt{B_n}\,\xi.
    """
    dt = T / M
...
def continuation_surface_from_coefficients(
    ...
    r"""Evaluate \widehat C_n(s_i,v,v') = a_n(s_i)\cdot\phi(v,v')."""
...
def fst_conditional_expectation_batch(
    ...
    *,
    ...
    r"""
    For each row j, compute on the log grid

        u_j(y_i) = E[f_j(y_i + m_j + \sqrt{q_j} Z)],  Z \sim N(0,1),

    by FFT convolution with constant edge padding.
    """
    ...
    for start in range(0, n_paths, batch_size):
        ...
        if np.any(~small_mask):
            ...
            out_chunk[idx] = conv[:, offset:offset + n_space]

        if np.any(small_mask):
            for local_idx in np.where(small_mask)[0]:
                out_chunk[local_idx] = _interp_shifted_same_grid(
                    log_grid,
                    chunk[local_idx],
...
def hybrid_prices() -> dict[str, float | int | bool]:
    ...
    coefficient_steps: list[np.ndarray | None] = [None] * (N_ex + 1)
