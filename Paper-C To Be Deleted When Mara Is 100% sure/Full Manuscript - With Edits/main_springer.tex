...
\title[Hybrid LSMC–PDE Method for Bermudan Option under the Gatheral Double Mean-Reverting Model]{Hybrid LSMC–PDE Method for Bermudan Option under the Gatheral Double Mean-Reverting Model}
...
\abstract{
We study Bermudan put pricing under the Gatheral Double Mean-Reverting (GDMR) stochastic volatility model. The model has two variance factors and allows Constant Elasticity of Variance (CEV)-type volatility exponents. This makes direct pricing more difficult than in standard one-factor models. We adapt the hybrid Least-Squares Monte Carlo--Partial Differential Equation (LSMC--PDE) method for the GDMR model. Conditional on simulated variance paths, the asset-price step becomes a one-dimensional pricing problem, solved by a Fourier-based method, while the remaining variance dependence is approximated by a least-squares regression. We also show that the variance system is strongly well-posed and remains nonnegative for CEV exponents $\delta_1,\delta_2\in[1/2,1]$. Finally, we give detailed description of numerical implementation and provide experiments showing that the hybrid LSMC--PDE estimator gives competitive prices and often smaller errors than plain LSMC, especially for low and moderate numbers of paths.
}
...
Mixed Monte Carlo--PDE methods reduce this difficulty by combining simulation
with conditional PDE calculations. Loeper and Pironneau~\cite{loeper2009mixed}
proposed a mixed PDE/Monte Carlo method for stochastic-volatility models.
Lipp, Loeper, and Pironneau~\cite{lipp2013mixing} developed related
Monte Carlo--PDE mixing ideas for option pricing. Cozma and
Reisinger~\cite{cozma2017mixed} used a mixed Monte Carlo--PDE variance
reduction method for foreign exchange options under a Heston-Cox--Ingersoll--Ross (Heston--CIR) model.
Farahany, Jackson, and Jaimungal~\cite{farahany2020mixing} extended this
line of work to Bermudan options by combining LSMC with conditional PDE
steps, which forms the basis of the present work. Their algorithm simulates the volatility process, solves conditional
...
We study Bermudan put pricing under the Gatheral-type double mean-reverting stochastic volatility (GDMR) model, introduced in Gatheral~\cite{gatheral2008}, where successful simultaneous calibration to both SPX and VIX option markets was demonstrated. Under this model, the instantaneous variance $v$ mean-reverts to a stochastic variance level $v'$, while $v'$ mean-reverts to a long-run level. This double mean-reverting structure is motivated by the
...
Despite its appealing modelling properties, the GDMR model does not admit closed-form pricing formulas, even for European options. As a result, calibration in Gatheral~\cite{gatheral2008} was performed using Euler simulation, while Bayer, Gatheral, and Karlsmark~\cite{bayer2013fast} later employed the higher-order Ninomiya--Victoir scheme to improve computational efficiency. Moreover, there has been limited research on Bermudan and American option pricing under this framework. To the best of our knowledge, \cite{haida2026almost} is among the few studies addressing this problem, proposing a simulation scheme together with a full Least Squares Monte Carlo (LSMC) method for American option pricing under a double-Heston-type parameterization of the GDMR model. In this work, we aim to fill this gap by developing a hybrid LSMC--PDE method, adapted from the more general framework of Farahany, Jackson, and Jaimungal~\cite{farahany2020mixing}. This approach is motivated by the computational challenges of applying a full PDE method in the presence of three state variables, while avoiding the need for a full regression approximation of the continuation value as in standard LSMC.
%% to be added back into a suitable place, it is very strange to appear here. /Ying


We note two advantages of this method relative to a plain LSMC method. First, in contrast to the plain LSMC, where the continuation value is entirely represented through a parametric regression ansatz, the proposed hybrid framework performs only a partial approximation, leaving the stock-price dependence to be resolved through a conditional pricing equation. By reducing the extent of statistical approximation in continuation-value estimation, the method is expected to provide a more faithful representation of the exercise decision, particularly in settings where the early exercise boundary and stable Greeks are important.

Secondly, the hybrid method naturally produces option values on a grid of stock prices through the conditional PDE step. In contrast, a standard plain LSMC implementation typically provides an estimate for a single initial stock price and must be rerun for alternative initial conditions. For a fixed strike, a grid of stock prices is equivalent to a collection of option prices across different moneyness levels. This feature is particularly advantageous for model calibration, where option prices across multiple moneyness values and maturities must be computed repeatedly.

The present paper therefore specializes the hybrid LSMC–PDE framework to the GDMR model, motivated by the considerations discussed above.  This adoption is not automatic for the Gatheral double mean-reverting model, because the model has two correlated variance Brownian drivers and Constant Elasticity of Variance (CEV)--type variance coefficients. Our theoretical contribution therefore consists of a model-specific adaptation of an existing hybrid method rather than a new general Hybrid LSMC--PDE algorithm. First, we derive the Brownian projection and the conditional one-step Gaussian representation needed to apply the existing Hybrid LSMC--PDE framework to the Gatheral double mean-reverting model. The key model-specific quantities are the correlated Brownian shift $Z_n$, the integrated drift $I_n$, the integrated residual variance $B_n$, and the terminal volatility
state.  

While the main focus of this paper is the development and numerical validation of the hybrid LSMC–PDE method under the GDMR model, we additionally provide a theoretical analysis of model well-posedness under suitable conditions. 
We also include a well-posedness and nonnegativity argument for the non-reflected GDMR dynamics under $\delta_1,\delta_2\in[1/2,1]$. This argument records the state-space property needed for the Bermudan recursion and
for the conditional construction used later. Such analysis is relevant since the GDMR model allows for CEV--type parametrizations, leading to non-Lipschitz diffusion coefficients and potential boundary-related issues. The CEV diffusion was originally introduced in the option pricing context by John C. Cox \cite{cox1975cev}, while Chan, Karolyi, Longstaff, and Sanders \cite{chan1992empirical} studied related CEV--type specifications in interest-rate modelling. Foundational pathwise uniqueness results for one-dimensional stochastic differential equations with non-Lipschitz diffusion coefficients were established by Yamada and Watanabe \cite{yamada1971uniqueness}. More recently, Mishura, Pilipenko, and Ralchenko \cite{mishura2025gatheral} investigated well-posedness and boundary behavior for a related Gatheral double stochastic-volatility model with Skorokhod reflection. These results are relevant because the CEV generalization creates a boundary issue at zero. For exponents below one, the variance diffusion coefficients are not Lipschitz at the boundary, while the model is meaningful only if the variance factors remain nonnegative.


From a computational perspective, we also provide a detailed description of the (FFT)-based  numerical implementation and extensive numerical experimental results, which may serve as benchmark values for future studies. For the numerical experiments, we use a fixed parameter set based on a calibrated double mean-reverting specification from
Bayer, Gatheral and Karlsmark~\citep{bayer2013fast}. We compare plain LSMC and the Hybrid LSMC--PDE estimator by varying both the Euler step count and the Monte Carlo path budget. Since no closed-form Bermudan option price is available under the GDMR model, the comparisons are made against large-simulation plain LSMC reference prices.
...
$$
be a filtered probability space satisfying the usual conditions.  Throughout the paper, option pricing is performed under a risk-neutral (equivalent martingale) measure $\Q$ under which the discounted stock price process is assumed to be a true martingale. We assume a deterministic money-market account $B_t=e^{rt}$. Unless otherwise specified, all expectations are taken with respect to the probability measure $\Q$. Furthermore, the notation $\E_\xi[\cdot]$ denotes expectation with respect to the probability distribution of the random variable $\xi$.
...
Allowing CEV exponents in the variance specification creates a boundary issue at zero, because the coefficients are not Lipschitz at the boundary when $\delta_i<1$. 

Mishura, Pilipenko, and Ralchenko~\cite{mishura2025gatheral} studied the GDMR model with CEV exponents. They prove strong existence, uniqueness, nonnegativity, and the strong Markov property for strictly positive deterministic initial values, and they further analyze near-zero behavior and a Skorokhod-reflected modification. We include the following result to make the closed-state-space property explicit for the GDMR dynamics used in this paper. In particular, we allow $v_0=0$ and $v'_0=0$, although the numerical pricing experiments below use strictly positive initial variance factors.
...
Here, we allow the boundary cases $v_0=0$ and $v'_0=0$. These cases are included only to prove that the variance system is well defined on the closed nonnegative domain and cannot leave it. We do not treat them as a modelling choice for calibration or numerical pricing. In the pricing part of the paper, the initial variance factors are taken to be strictly positive.

The proof is given in Appendix~\ref{app:proof-wellposed}. The proof follows the triangular structure of the model.
...
general hybrid-regression convergence theorem of Farahany et al.~\cite{farahany2020mixing}; the representation derived here provides the
...
The almost-sure convergence theorem for the Hybrid LSMC--PDE regression is proved in \cite{farahany2020mixing}. The preceding subsections identify the model-specific ingredients needed to apply that theorem.
...
the grids, variance discretization, accumulated path statistics, FFT step, regression step, and time-zero estimator. This numerical choice is natural since the conditional PDE admits a Gaussian structure in its solution, allowing the shift operation in the conditional formulation to be handled efficiently in Fourier space. In addition, the FFT implementation appears computationally advantageous in our numerical experiments.
...
\right\}.
\end{equation}
Note that the present implementation has the advantage that option prices are computed simultaneously for multiple initial stock prices on the stock-price grid, equivalently for multiple moneyness levels, which is particularly desirable for calibration. In contrast, a plain LSMC approach typically produces the option price corresponding to a single initial stock price in each run.
...
We report Bermudan put pricing experiments for the fixed parameter set in Table~\ref{tab:parameter-setup}. Since no closed-form solution is available for Bermudan option prices under the GDMR model, we compare a plain LSMC estimator with our Hybrid LSMC--PDE estimator, using large-simulation plain LSMC estimates as reference prices. The plain LSMC estimator
\cite{longstaff2001valuing} and the Hybrid LSMC--PDE estimator in Eq.~\eqref{eq:hybrid_est} are compared using reference-relative errors. Standard errors and 95\% confidence intervals are reported for the reference prices and used to construct the error bars in the figures.
...
made against the LSMC reference values in Table~\ref{tab:reference-prices}, not exact prices. The complete set of estimated option prices is reported in Appendix~\ref{app:price-grids} and may serve as benchmark values for future studies.

\section{Conclusion}\label{sec:conclusion}

In this paper, we studied Bermudan put pricing under a GDMR model. Bermudan pricing requires the approximation of continuation values at each exercise date, and the two variance factors in the GDMR model make this regression problem more difficult than in a one-factor model. At the same time, the model has a useful one-way coupling; the variance factors can be simulated independently of the stock price. This structure leads naturally to the Hybrid LSMC--PDE method, since the stock-price direction can be handled conditionally while the remaining variance dependence is approximated by least-squares regression. We condition on the Brownian paths driving the variance factors and project the asset Brownian motion onto these drivers. Under this conditioning, each continuation step reduces to a one-dimensional problem in log-price. This gives the Gaussian conditional representation based on $Z_n$, $I_n$, $B_n$, and the variance values at the next exercise date, and leads to an FFT-based implementation of the conditional asset step. We also included that, for CEV exponents $\delta_1,\delta_2 \in [1/2,1]$, the non-reflected GDMR variance system has a unique strong solution and remains nonnegative. This property is needed since the construction of Hybrid LSMC--PDE relies on valid simulated variance paths.

We do the numerical study using a calibrated parameter set from the literature. We compared plain LSMC with the Hybrid LSMC--PDE estimator across different Euler step counts and different numbers of Monte Carlo paths. Because no closed-form Bermudan benchmark is available for the GDMR model, the comparison was made against large-simulation plain LSMC reference prices. Within this setup, the Hybrid LSMC--PDE estimator produced smaller reference-relative errors in many of the reported cases. The clearest gains appeared when the number of paths was low or moderate. These results suggest that the conditional PDE step can make the continuation regression less demanding in this model.

For the future work we can test the method under additional calibrated parameter sets, consider alternative reference prices or bias-control procedures such as low-biased or dual estimators, and study other payoff types and early-exercise contracts.
...
$$
Thus the diffusion has no outward noise at the boundary and the drift points inward on the negative side. TThe standard one-dimensional stochastic invariance criterion for closed intervals therefore yields
$$
