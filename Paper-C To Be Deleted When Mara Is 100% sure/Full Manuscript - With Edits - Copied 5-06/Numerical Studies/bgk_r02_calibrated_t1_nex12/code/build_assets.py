...
def render_path_figure(config: dict[str, Any], euler_steps: int, manifest: list[dict[str, str]]) -> None:
    ...
    delta_value = float(config["model_env"]["GDMR_DELTA1"])
    legend_axis.text(0.02, 0.58, f"Case: r={r_value:.2f}, v0={v0_value:.3f}, delta1=delta2={delta_value:.2f}", ha="left", va="top", fontsize=10)
    legend_axis.text(0.02, 0.40, "Reference: LSMC, 1200 steps, 1.2M paths", ha="left", va="top", fontsize=10)
...
def write_experimental_setting_table(config: dict[str, Any], manifest: list[dict[str, str]]) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Model and numerical parameters for the standalone calibrated positive-rate study.}",
