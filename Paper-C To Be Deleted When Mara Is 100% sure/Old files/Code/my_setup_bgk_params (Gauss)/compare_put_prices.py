...
def scenario_label_and_slug(spot, strike):
    if abs(strike - spot) <= 1e-12:
        return "ATM put", "atm"
    if strike > spot:
        return "ITM put", "itm"
    return "OTM put", "otm"
...
def draw_svg(summary):
    ...
    lines = [
        ...
        f'<text x="70" y="118" font-family="{title_font}" font-size="15" fill="#475569">BGK parameter block applied to the my_setup_delta_075 Gauss-Hermite gDMR workflow</text>',
        ...
        f'<text x="1049" y="165" text-anchor="middle" font-family="{title_font}" font-size="12" font-weight="700" fill="#ffffff">{pass_text} {100.0 * direct_target:.2f}%</text>',
    ...
    lines.append('</svg>')
    svg_text = "\n".join(lines)
    graph_path.write_text(svg_text, encoding="utf-8")
    scenario_path = this_folder / f"compare_put_prices_{summary['scenario_slug']}.svg"
    scenario_path.write_text(svg_text, encoding="utf-8")
...
summary["scenario_label"], summary["scenario_slug"] = scenario_label_and_slug(
    summary["spot"],
    summary["strike"],
)

scenario_graph_path = draw_svg(summary)
...
print(f"{100.0 * direct_target:.0f}% direct target met:          {'yes' if summary['direct_target_met'] else 'no'}")
print("Result:                        LSMC direct is the headline reference for this standalone gDMR comparison.")
print(f"Scenario:                      {summary['scenario_label']}")
print(f"Graph saved to:                {graph_path}")
print(f"Scenario graph saved to:       {scenario_graph_path}")
