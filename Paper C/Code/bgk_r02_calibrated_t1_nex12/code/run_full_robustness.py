...
LSMC_HELPER = CODE_DIR / "scratch_lsmc_helpers.py"
HYBRID_HELPER = CODE_DIR / "scratch_hybrid_helpers.py"
...
if CASE_ID != RUN_ROOT.name:
    ...
    data_config_path = DATA_ROOT / "config" / "rerun_config.json"
...
def write_metadata(config: dict[str, Any]) -> None:
    metadata = {
        ...
        "run_root": package_relative(RUN_ROOT),
        "data_root": package_relative(DATA_ROOT),
        "sandbox_only": True,
        ...
        "engines": {
            "lsmc": package_relative(LSMC_SCRIPT),
            "hybrid": package_relative(HYBRID_SCRIPT),
            "lsmc_helper": package_relative(LSMC_HELPER),
            "hybrid_helper": package_relative(HYBRID_HELPER),
            "config": package_relative(CONFIG_PATH),
            "lsmc_sha256": file_hash(LSMC_SCRIPT),
            "hybrid_sha256": file_hash(HYBRID_SCRIPT),
            "lsmc_helper_sha256": file_hash(LSMC_HELPER),
            "hybrid_helper_sha256": file_hash(HYBRID_HELPER),
...
def model_fingerprint(result: dict[str, Any]) -> dict[str, Any]:
    return {
        "S0": f"{finite(result['S0']):.12g}",
        "T": f"{finite(result['T']):.12g}",
        "r": f"{finite(result['r']):.12g}",
        "v0": f"{finite(result['v0']):.12g}",
        "vp0": f"{finite(result['vp0']):.12g}",
        "kappa1": f"{finite(result['kappa1']):.12g}",
        "kappa2": f"{finite(result['kappa2']):.12g}",
        "theta": f"{finite(result['theta']):.12g}",
        "xi1": f"{finite(result['xi1']):.12g}",
        "xi2": f"{finite(result['xi2']):.12g}",
        "rho12": f"{finite(result['rho12']):.12g}",
        "rho13": f"{finite(result['rho13']):.12g}",
        "rho23": f"{finite(result['rho23']):.12g}",
...
def run_smoke(config: dict[str, Any], force: bool) -> None:
    ...
    for method, paths_key in (("benchmark", "lsmc_paths"), ("hybrid", "hybrid_paths")):
        ...
        row = {
            ...
            "se_direct": f"{se:.12f}",
            **model_fingerprint(result),
            "runtime_seconds": f"{runtime:.6f}",
            "engine_sha256": engine_sha,
            "scratch_path": package_relative(job_scratch_path(method, strike, int(config["smoke"]["euler_steps"]), paths)),
            "log_path": package_relative(job_log_path(method, strike, int(config["smoke"]["euler_steps"]), paths)),
        }
        ...
        append_manifest(
            {
                ...
                "paths": int(result["paths"]),
                "method": "benchmark",
                "output_path": package_relative(BENCHMARK_CSV),
                "runtime_seconds": f"{runtime:.6f}",
            }
        )
                    ...
                    append_manifest(
                        {
                            ...
                            "paths": int(path_budget),
                            "method": method,
                            "output_path": package_relative(output_path),
                            "runtime_seconds": f"{runtime:.6f}",
                        }
                    )
...
def main() -> None:
    ...
    parser.add_argument("--mode", choices=["smoke", "benchmark", "step", "path", "assets", "full"], default="full")
